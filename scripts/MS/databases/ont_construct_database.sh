...
#SBATCH --output=ont_construct_database-%j.out

###############################################################

# Script Name:        ont_construct_database.sh
# Author:             Aidan Tay
# Version:            v1.00
...
#              transcriptome assemblies from Oxford Nanopore MinION
...
constructDatabase() {
    ...
    local inputFilePrefix=$(basename ${inputFilepath} | sed "s|_guppy.*.gtf||g")
    ...
    rm $DATABASES_OUTPUT_DIR/tempOnt.gtf
}
...
formatGtf() {
    ...
    | sed 's/^/chr/g' \
    > $DATABASES_OUTPUT_DIR/tempOnt.gtf

    echo $DATABASES_OUTPUT_DIR/tempOnt.gtf
}
...
constructDatabase $TRANSCRIPTS_OUTPUT_DIR/ont_guppy_merged_nanofilt_minimap_filtered_sorted_stringtie.gtf
